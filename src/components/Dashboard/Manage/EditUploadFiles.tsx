...
import useEditUploadFileStore from '../../../hooks/useEditUploadFileStore';
import {
  copyTextToClipboard,
  getRelativeSize,
  isBrowser,
          ...
          <When condition={!props.hideRemoveButton}>
            <Box px={0} ml="auto" w="22px">
              <IconButton
                ...
                onClick={() => removeFile(file.name)} // specify actual name
              />
            </Box>
          </When>
        </Flex>
...
function UploadFile(props: {
  ...
  const upload = async () => {
    try {
      ...
      setUploadErr('');
      setIsUploading(() => true);
      const { uploadUrls: urls, upload } = await getUpdateUploadLinks(
        id,
        props.upload._id,
        {
          files: files,
      ...
      if (urls.length === 0) {
        ...
        setDownloadLink(
          () =>
            `${isBrowser() ? window.location.origin : ''}/download?uploadId=${
              upload._id
            }`,
        );
        setStep(() => 'enjoy');
      ...
      for (let i = 0; i < urls.length; i += 1) {
        try {
          await axios.put(urls[i].url, files[i], {
            headers: {
            ...
            onUploadProgress: (event) => {
              ...
              if (percent >= 100) {
                validateUploadCompletion(
                  id,
                  upload._id,
                  files.map((f, i) => ({ name: urls[i].name, size: f.size })),
                )
                  .then(() => {
                    setIsUploading(() => false);
                    setDownloadLink(
  ...
  const allFiles = [...props.upload.files, ...files].filter(
    (f) => !deletedFiles.includes(f.name),
  );

  const nextStep = () => {
    if (step === 'upload' && allFiles.length > 0) return setStep('customize');
    if (step === 'customize') {
      return upload();
    }
    if (step === 'enjoy') {
  ...
  React.useEffect(() => {
    ...
    } else if (props.upload.coverImage)
      setImgUrl(
        `${process.env.GATSBY_BACKEND_URL}/uploads/${props.upload?._id}/cover-image`,
      );
  }, [image]);
  ...
  return (
    <Modal isOpen={props.isOpen || isUploading} onClose={props.onClose}>
      <ModalContent p={0} m={0} maxW="full">
        <ModalBody p={0}>
          <Box
          ...
          >
              ...
              <Switch>
                ...
                <Case condition={step === 'enjoy'}>
                  ...
                  >
                    ...
                    <FilesTable files={props.upload.files} hideRemoveButton />
            ...
            <Box
              ...
              bgSize="cover"
            >
              <When condition={!image || !props.upload.coverImage}>
                <Box
                  className="flex items-center justify-center"
...
export default function EditUploadFiles(props: {
  workspaceId?: string;
  upload: {
    _id: string;
    color?: string;
  ...
  return (
    <React.Fragment>
      <MenuItem onClick={() => props.onOpen()}>{t('edit')}</MenuItem>
